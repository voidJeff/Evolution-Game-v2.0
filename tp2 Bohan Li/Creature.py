...
class Creature(object):
	...
	def draw(self, canvas):
		...
		color = getCreatureColor(100, self.color)
...
class Predator(Creature):
	...
	def move(self):
		#self.rotate(random.choice([-1.5,1.5]))
		self.cx += math.cos(math.radians(self.direction))*self.currentSpeed
		self.cy -= math.sin(math.radians(self.direction))*self.currentSpeed
	...
	def detectFood(self, foodList):
		...
		if distance(self.cx, self.cy, food.cx, food.cy) < 15:
			self.direction = foodAngle
		...
		delt = self.agility/3 + 3
		if foodAngle > self.direction:
			if foodAngle - self.direction > 180:
				self.currentSpeed = (foodAngle - self.direction - 180)/180 * self.speed +1
				delt += ((foodAngle - self.direction - 180)/180)**0.5 * 2
				if abs((foodAngle - self.direction - 180) - 90) < -20: 
					delt = 3
					self.currentSpeed = 1
				self.rotate(-delt)
			elif foodAngle - self.direction < 180:
				self.currentSpeed = (180 - foodAngle + self.direction)/180 * self.speed +1
				delt += ((foodAngle - self.direction)/180)**0.5 * 2
				if abs((foodAngle - self.direction) - 90) < -20: 
					delt = 3
					self.currentSpeed = 1
				self.rotate(delt)
			return foodInSight
			...
			if self.direction - foodAngle > 180:
				self.currentSpeed = (self.direction - foodAngle - 180)/180 * self.speed + 1
				delt += ((self.direction - foodAngle - 180)/180)**0.5 * 2
				if abs((self.direction - foodAngle - 180) - 90) < -20: 
					delt = 3
					self.currentSpeed = 1
				self.rotate(delt)
			elif self.direction - foodAngle < 180:
				self.currentSpeed = (180 - self.direction + foodAngle)/180 * self.speed + 1 
				delt += ((self.direction - foodAngle )/180)**0.5 * 2
				if abs((self.direction - foodAngle) - 90) < -20: 
					delt = 3
					self.currentSpeed = 1
				self.rotate(-delt)
			return foodInSight
...
def spawnNewCreatures(data, cx, cy, color, playerNum):
	if playerNum == 1:
		data.creatureList.append(Predator(cx, cy, 2+data.speed/4, data.health*20, 150*data.vision, color, data.agility, data.sex))
	elif playerNum == 2:
		data.creatureList1.append(Predator(cx, cy, 2+data.speed1/4, data.health1*20, 150*data.vision1, color, data.agility1, data.sex1))
