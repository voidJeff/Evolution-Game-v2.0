...
def fitness(data):
	...
	#for creature in data.creatureList:
		#lst.append(creature.health)
	#print(lst)

def reproduce(creature):
	dna = creature.getDNA()
	dna = mutate(dna)
	return Predator(creature.cx,creature.cy,2+dna[0]/4,dna[1]*20,dna[2]*150,creature.color,dna[3],dna[4])
...
def nextGeneration(data):
	...
	for creature in data.reproducePool:
		count = int(creature.children)
		if random.uniform(0,1) < creature.children - count: count += 1
